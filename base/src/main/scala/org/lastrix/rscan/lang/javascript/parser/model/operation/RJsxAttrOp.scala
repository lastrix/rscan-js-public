...
sealed class RJsxAttrOp private
(
  key: ROpKey,
  val attrName: String,
  children: Seq[ROp]
  ...
  override def prettyPrint(sb: StringBuilder, prefix: String): Unit = {
    sb.append(prefix).append(key).append(LINE_SEPARATOR)
      .append(prefix).append("\t").append("AttributeName: ").append(attrName).append(LINE_SEPARATOR)
    appendChildren(sb, prefix)
  }
}

object RJsxAttrOp {
  def apply(statement: Statement, attrName: String, children: Seq[ROp]): RJsxAttrOp =
    new RJsxAttrOp(ROpKey(JsOpType.RAW_JSX_ATTRIBUTE, statement), attrName, children)
